...
const GRID_GAP = 16          // web `.community-panel { gap: 16px }`
const GRID_H_PAD = spacing.lg // body side padding (each edge)
...
export default function CommunityScreen() {
    ...
    // Fluid masonry grid — mirrors the web `.community-panel` (repeat(auto-fill, minmax(260px,1fr))).
    // Column count fans out with the available content width; the item width is derived from the same
    // width so a partial final row doesn't stretch (auto-fill leaves the track empty instead).
    const { width: windowWidth } = useWindowDimensions()
    const breakpoint = useBreakpoint()
    const collapsed = useSidebarStore(s => s.collapsed)
    const isWide = breakpoint === 'tabletLandscape' || breakpoint === 'desktop'
    const rail = isWide ? (collapsed ? 68 : 280) : 0
    const gridCols = useGridColumns(260, { gap: GRID_GAP, horizontalPadding: GRID_H_PAD * 2 })
    const contentWidth = Math.max(0, windowWidth - rail - GRID_H_PAD * 2)
    const itemWidth = gridCols > 1 ? Math.floor((contentWidth - GRID_GAP * (gridCols - 1)) / gridCols) : contentWidth
        ...
        <View style={styles.container}>
            <View style={styles.header}>
                <View style={styles.headerBody}>
                    ...
                    <Text style={[styles.title, !isWide && styles.titleNarrow]} numberOfLines={2}>{gameName ?? ''}</Text>
            ...
            {sources.length === 0 && !pendingSub ? (
                <View style={styles.empty}>
                    ...
                    <Pressable style={[styles.manageBtn, styles.manageBtnRow]} onPress={openModal}>
                        <Lucide name="settings" color={colors.iconMuted} size={15} />
                ...
                <>
                    <View style={styles.tabsBar}>
                        ...
                        <Pressable style={styles.manageToggle} onPress={openModal}><Lucide name="settings" color={colors.iconMuted} size={15} /></Pressable>
                    </View>
                    ...
                    {activeTab === '__pending__' && pendingSub ? (
                        ...
                        <FlatList
                            // Remount when the column count changes (rail collapse / rotation) — the
                            // documented way to re-flow a numColumns FlatList.
                            key={`cols-${gridCols}`}
                            data={visiblePosts}
                            keyExtractor={(p) => p.id}
                            numColumns={gridCols}
                            ...
                            renderItem={({ item }) => (
                                <PostCard
                                    post={item} appid={appid} prefs={prefs} apiHost={apiHost} width={itemWidth}
...
// Port of Breadcrumb.svelte — the pill trail (Home › Game › Community). Horizontally scrollable so a
// long game name never forces the header to overflow (matches the web's ≤479px scroll-strip fallback).
...
const styles = StyleSheet.create({
    ...
    // `.community-header` — hero band: breadcrumb + large serif title on the left, pin badge on the right.
    header: {
        flexDirection: 'row', alignItems: 'flex-start', gap: spacing.md,
        paddingHorizontal: spacing.lg, paddingTop: spacing.lg, paddingBottom: spacing.md,
        borderBottomWidth: 1, borderBottomColor: colors.border,
    },
    headerBody: { flex: 1, minWidth: 0, gap: spacing.sm },
    // `.community-title` — 2.4rem serif hero (shrinks on phone, matching the web @479px step-down).
    title: { color: colors.text, fontFamily: fonts.title, fontSize: 32, lineHeight: 38, letterSpacing: 0.5 },
    titleNarrow: { fontSize: 22, lineHeight: 28 },
    subtitle: { color: colors.textMuted, fontFamily: fonts.ui, fontSize: 13 },
    // Masonry grid — vertical gap between rows + horizontal gap within a row (align-items:start keeps
    // cards their natural height, top-aligned, like the web CSS grid).
    gridContent: { paddingHorizontal: GRID_H_PAD, paddingTop: spacing.xs, paddingBottom: spacing.xl, gap: GRID_GAP },
    gridRow: { gap: GRID_GAP, alignItems: 'flex-start' },
    ...
    manageBtnRow: { flexDirection: 'row', alignItems: 'center', gap: 6 },
