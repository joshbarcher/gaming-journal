...
// web guide content gutter is 44px on desktop; container already pads spacing.lg, so add the remainder.
    ...
    return (
        <View style={styles.container}>
            ...
            <View style={[styles.body, isWide && styles.bodyWide]}>
                <View style={[styles.mainCol, isWide && { paddingHorizontal: GUTTER_WIDE - spacing.lg }]}>
                ...
                {/* Persistent Contents column at tablet/desktop — the web keeps the full nav tree +
                    Pins always visible; phone uses the summoned bottom-sheet instead. */}
                {isWide && (
...
// Port of Breadcrumb.svelte — pill trail (Home › Game › Journal › Guides › {title}). Horizontally
// scrollable so a long trail never overflows the header.
...
const styles = StyleSheet.create({
    ...
    // Header row: breadcrumb (left) + "Contents" heading aligned over the persistent panel (right).
    headerRow: { flexDirection: 'row', alignItems: 'flex-start', marginBottom: spacing.md },
    headerMain: { flex: 1, minWidth: 0 },
    headerToc: { width: TOC_PANEL_WIDTH, paddingLeft: spacing.lg },
    contentsHeading: { color: colors.text, fontFamily: fonts.title, fontSize: 22, letterSpacing: 0.5 },
    // Body: single column on phone; main content + persistent Contents column on tablet/desktop.
    body: { flex: 1 },
    bodyWide: { flexDirection: 'row', gap: spacing.xl },
    mainCol: { flex: 1, minWidth: 0 },
    tocPanel: { width: TOC_PANEL_WIDTH, borderLeftWidth: 1, borderLeftColor: colors.border, paddingLeft: spacing.lg },
