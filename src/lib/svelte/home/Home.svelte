<script lang="ts">
    ...
    const saleGame   = $derived(data.saleGame)
    ...
    const conditionals  = $derived([release, saleGame, resume].filter(Boolean))
    const hasConditional = $derived(conditionals.length > 0)
</script>
