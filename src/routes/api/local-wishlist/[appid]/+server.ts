...
async function relayPost(path: string) {
    ...
    try {
        ...
        logger.warn('[local-wishlist] relay call network error', { url, err: (err as Error).message })
    }
}
...
export async function POST({ params }: { params: { appid: string } }) {
    try {
        const appid = Number(params.appid)
        ...
        logger.error('[local-wishlist] POST error', { err: (err as Error).message })
        return json({ error: (err as Error).message }, { status: 500 })
    }
}
...
export async function DELETE({ params }: { params: { appid: string } }) {
    try {
        ...
        logger.error('[local-wishlist] DELETE error', { err: (err as Error).message })
        return json({ error: (err as Error).message }, { status: 500 })
    }
}
