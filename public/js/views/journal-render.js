...
export const RATING_LBLS  = { story: 'Story', soundMusic: 'Sound & Music', gameplay: 'Gameplay', graphics: 'Graphics', replayability: 'Replayability', performance: 'Performance', agendaFree: 'Agenda-Free' }
...
export function cleanAchName(apiname) {
    return apiname
        .replace(/^[A-Z0-9]+_/, '')
        .replace(/_/g, ' ')
        .toLowerCase()
...
export function mergeSessionAchievements(achList, achievementsDuring) {
...
export function renderSessionAchs(achs, achMap, noDataMsg = 'No achievements this session') {
...
export function renderAchItem(a, isUnlocked, isHidden) {
    const name        = a.displayName ?? cleanAchName(a.apiname)
...
export function renderLastSessionCard(sessions, displayAchList, appid) {
...
export function renderSessionHistoryRail(closedSessions) {
