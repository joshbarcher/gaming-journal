// Pure HTML-rendering functions extracted from game.js.
// No DOM manipulation, no event listeners — safe to call in {#html} blocks.
import { escapeHtml } from '../utils.js'

// ── Release status ────────────────────────────────────────────────────────────

export function releaseStatus(game) {
...
export function releaseBanner(game) {
...
export function fmtPlayerCount(n) {
    if (!n) return '—'
...
export function scoreChip(source, score, display, id = '') {
...
export function renderAbout(game) {
...
export function renderTrailers(appid, trailers) {
...
export function initTrailers(container) {
...
export function renderScreenshots(game) {
...
export function renderMyReview(entry) {
    ...
    const helpful     = r.votes_up > 0 ? `${fmtCount(r.votes_up)} found helpful` : null
...
export function initSteamReview(container) {
...
function _reviewCard(r) {
    ...
    const helpful  = r.votesUp > 0 ? `${fmtCount(r.votesUp)} found helpful` : ''
...
export function renderCommunityReviews(data, game) {
    if (releaseStatus(game) === 'coming_soon') return ''

    const section = (body) => `<section class="game-section" id="game-sec-community-reviews"><h2 class="game-section-title">Community Reviews</h2>${body}</section>`
    ...
    const summaryHtml = s && data.totalReviews > 0 ? `
...
export function renderItad(itad, game) {
...
export function renderGdpPrices(itad, game) {
    const inner = []
...
export function newsBBCodeDirty(news) {
    return news?.items?.some(item => BB_RE.test(item.contents ?? ''))
}
...
function _protonCap(s) { return s ? s.charAt(0).toUpperCase() + s.slice(1) : null }
function _protonLogPct(n) { return (Math.log(Math.max(1, Math.min(n, 5000))) / Math.log(5000) * 100).toFixed(1) }

export function renderProtonBadge(protonData, appid) {
...
export function renderProtondb(protonData, game) {
    if (!protonData?.tier) return ''
    const tier     = protonData.tier
...
function _pathCard(title, icon, pathObj) {
...
export function renderPcgw(pcgwData, game) {
